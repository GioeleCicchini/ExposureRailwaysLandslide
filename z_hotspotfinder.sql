...
AS $$
  ...
  BEGIN
    ...
    CREATE TEMP TABLE hotspottmp ( id serial PRIMARY KEY, id_station INTEGER, geom Geometry, zk FLOAT  ) ON COMMIT DROP;
    ...
    FOR particella IN SELECT * FROM dataset LOOP
      IF ST_Intersects(buffer, particella.geom) THEN
        INSERT INTO hotspottmp (id_station, geom) VALUES(station.gid, ST_Intersection(buffer, particella.geom) );
